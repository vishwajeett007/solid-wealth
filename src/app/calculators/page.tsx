    ...
    return (<div className="mb-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 sm:gap-0 mb-3">
        ...
        <div className="bg-[#fff9eb] px-3 py-1.5 rounded-lg flex items-center justify-between sm:justify-end border border-orange-100 sm:min-w-[120px]">
          ...
          <input type="number" value={value} onChange={handleInputChange} onBlur={handleBlur} className="bg-transparent border-none outline-none font-bold text-[#fe9800] text-sm w-full text-right sm:text-right p-0 m-0 focus:ring-0" style={{ appearance: "textfield", MozAppearance: "textfield" }}/>
          {suffix && <span className="text-[#fe9800] font-bold text-sm ml-1">{suffix}</span>}
        </div>
      </div>
      <input type="range" min={min} max={max} step={step} value={value} onChange={(e) => onChange(Number(e.target.value))} className="w-full h-1.5 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-[#fe9800] hover:accent-[#e58900] transition-all"/>
...
const ChartRenderer = ({ type, val1, val2, label1, label2, totalLabel, totalValue, subtext }: any) => {
    ...
    const CustomTooltip = ({ active, payload, label }: any) => {
        if (active && payload && payload.length) {
            return (<div className="bg-white p-3 rounded-xl shadow-xl border border-gray-100 text-sm min-w-[200px]">
          <p className="font-bold text-[#1a2332] mb-3 border-b border-gray-100 pb-2">{label}</p>
          {payload.map((entry: any, index: number) => (<div key={index} className="flex justify-between items-center gap-6 mb-2">
...
const CalculatorLayout = ({ inputs, reportInputs, val1, val2, label1, label2, totalLabel, totalValue, subtext, results, activeCalc, calculatorsList, setActiveCalcId }: any) => {
    ...
    return (<div className="w-full flex flex-col gap-6">
        ...
        <div className="w-full lg:w-[380px] flex-shrink-0 flex flex-col gap-6 print:hidden">
          ...
          <div className="bg-white rounded-[24px] border border-gray-100 p-6 md:p-8 shadow-sm">
            ...
            <div className="mt-8 space-y-3">
              {results.map((r: any, i: number) => (<div key={i} className={cn("flex justify-between items-center rounded-xl px-4 py-3 border border-gray-100", r.highlight ? "bg-[#fff9eb] border-orange-100" : "bg-gray-50/50")}>
        ...
        <div className="flex-1 w-full flex flex-col gap-6 print:hidden">
          ...
          <div className="bg-white rounded-[24px] border border-gray-100 p-6 md:p-8 shadow-sm flex flex-col min-h-[450px]">
            ...
            <div className="flex-1 flex flex-col justify-center">
              <ChartRenderer type={chartType} val1={val1} val2={val2} label1={label1} label2={label2} totalLabel={totalLabel} totalValue={totalValue} subtext={subtext}/>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[24px] border border-gray-100 p-6 md:p-8 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4">
              {calculatorsList.filter((c: any) => c.id !== activeCalc.id).map((calc: any) => (<button key={calc.id} onClick={() => setActiveCalcId(calc.id)} className="flex items-center justify-between px-5 py-4 rounded-xl border border-gray-100 hover:border-[#fe9800]/30 hover:bg-orange-50/30 transition-colors text-left group bg-gray-50/50">
      ...
      {showPreview && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-2 sm:p-4 print:p-0 print:bg-white print:relative print:block print:inset-auto" onClick={() => setShowPreview(false)}>
                  ...
                  <div className="flex flex-col sm:flex-row gap-16 w-full">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="space-y-4">
                        {reportInputs && reportInputs.map((r: any, i: number) => (<div key={i} className="flex justify-between items-center py-2 border-b border-gray-50">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="space-y-4">
                        {results.map((r: any, i: number) => (<div key={i} className={cn("flex justify-between items-center py-2 border-b", r.highlight ? "border-orange-200 bg-orange-50/50 -mx-3 px-3 rounded-lg" : "border-gray-50")}>
...
const EMICalculator = (props: any) => {
    ...
    return (<CalculatorLayout {...props} inputs={<>
        ...
        ]} val1={loanAmount} val2={totalInterest} label1="Principal" label2="Total Interest" totalLabel="Total Payment" totalValue={totalPayment} subtext={`over ${years} years (${n} EMIs)`}/>);
};
const SIPCalculator = (props: any) => {
    ...
    return (<CalculatorLayout {...props} inputs={<>
        ...
        ]} val1={invested} val2={wealthGained} label1="Invested Amount" label2="Est. Returns" totalLabel="Total Value" totalValue={futureValue} subtext={`over ${years} years`}/>);
};
const LumpsumCalculator = (props: any) => {
    ...
    return (<CalculatorLayout {...props} inputs={<><InputSlider label="Total investment" value={lumpsum} min={10000} max={10000000} step={10000} prefix="₹" onChange={setLumpsum}/><InputSlider label="Expected return (p.a)" value={rate} min={1} max={30} step={0.1} suffix="%" onChange={setRate}/><InputSlider label="Time period" value={years} min={1} max={40} step={1} suffix="Yr" onChange={setYears}/></>} reportInputs={[
            { label: "Total Investment", value: lumpsum },
            { label: "Expected Return (p.a.)", value: rate, suffix: "%", isCurrency: false },
            { label: "Time Period", value: years, suffix: "Years", isCurrency: false }
        ]} results={[{ label: "Invested Amount", value: lumpsum }, { label: "Est. Returns", value: futureValue - lumpsum }, { label: "Total Value", value: futureValue, highlight: true }]} val1={lumpsum} val2={futureValue - lumpsum} label1="Invested" label2="Returns" totalLabel="Total Value" totalValue={futureValue} subtext={`over ${years} years`}/>);
};
const SWPCalculator = (props: any) => {
    ...
    return (<CalculatorLayout {...props} inputs={<><InputSlider label="Total investment" value={corpus} min={100000} max={50000000} step={100000} prefix="₹" onChange={setCorpus}/><InputSlider label="Withdrawal per month" value={withdrawal} min={1000} max={500000} step={1000} prefix="₹" onChange={setWithdrawal}/><InputSlider label="Expected return (p.a)" value={rate} min={1} max={30} step={0.1} suffix="%" onChange={setRate}/><InputSlider label="Time period" value={years} min={1} max={40} step={1} suffix="Yr" onChange={setYears}/></>} reportInputs={[
            { label: "Total Investment", value: corpus },
            { label: "Withdrawal per month", value: withdrawal },
            { label: "Expected Return (p.a.)", value: rate, suffix: "%", isCurrency: false },
            { label: "Time Period", value: years, suffix: "Years", isCurrency: false }
        ]} results={[{ label: "Total Investment", value: corpus }, { label: "Total Withdrawal", value: totalWithdrawn }, { label: "Final Value", value: finalCorpus > 0 ? finalCorpus : 0, highlight: true }]} val1={finalCorpus > 0 ? finalCorpus : 0} val2={totalWithdrawn} label1="Final Value" label2="Total Withdrawn" totalLabel="Initial Corpus" totalValue={corpus}/>);
};
const StepUpSIPCalculator = (props: any) => {
    ...
    return (<CalculatorLayout {...props} inputs={<><InputSlider label="Monthly investment" value={initialSip} min={500} max={100000} step={500} prefix="₹" onChange={setInitialSip}/><InputSlider label="Annual step-up" value={stepUp} min={1} max={50} step={1} suffix="%" onChange={setStepUp}/><InputSlider label="Expected return (p.a)" value={rate} min={1} max={30} step={0.1} suffix="%" onChange={setRate}/><InputSlider label="Time period" value={years} min={1} max={40} step={1} suffix="Yr" onChange={setYears}/></>} reportInputs={[
            { label: "Monthly Investment", value: initialSip },
            { label: "Annual Step-up", value: stepUp, suffix: "%", isCurrency: false },
            { label: "Expected Return (p.a.)", value: rate, suffix: "%", isCurrency: false },
            { label: "Time Period", value: years, suffix: "Years", isCurrency: false }
        ]} results={[{ label: "Invested amount", value: invested }, { label: "Est. returns", value: fv - invested }, { label: "Total value", value: fv, highlight: true }]} val1={invested} val2={fv - invested} label1="Invested" label2="Returns" totalLabel="Total Value" totalValue={fv}/>);
};
...
export default function CalculatorsPage() {
    ...
    return (<div className="min-h-screen bg-[#FFFDF4] pt-24 sm:pt-28 pb-20 print:pt-0 print:pb-0 print:bg-white">
