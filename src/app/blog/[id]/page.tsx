...
import { ArrowLeft, Clock, Activity, FileText, AlertTriangle, ChevronRight, TrendingUp, Sparkles, BookOpen, Calendar, User, Share2, Copy, Check, BarChart4, PieChart as PieIcon, GitCommit, LayoutGrid } from "lucide-react";
interface Blog {
    id: string;
    title: string;
    summary: string;
...
export default function BlogDetailPage() {
    ...
    if (!mounted || !blog) {
        return (<div className="flex min-h-screen items-center justify-center bg-wealth-bg font-sans">
    ...
    return (<div className="min-h-screen bg-wealth-bg font-sans text-wealth-primary pb-24">
      ...
      <div className="max-w-6xl mx-auto px-4 pt-8 md:pt-12">
        ...
        <div className="flex items-center justify-between mb-8">
          <button onClick={() => router.push("/blog")} className="inline-flex items-center gap-1.5 px-4 py-2 border border-wealth-border bg-white rounded-xl text-xs font-bold hover:bg-wealth-surface-dim transition-all text-wealth-secondary cursor-pointer shadow-wealth-sm">
            <ArrowLeft className="w-4 h-4"/> Back to Discover Hub
          </button>
          
          <div className="flex items-center gap-2">
            <button onClick={handleCopyLink} className="inline-flex items-center gap-1 px-3 py-2 border border-wealth-border bg-white rounded-xl text-xs font-bold hover:bg-wealth-surface-dim transition-all text-wealth-secondary cursor-pointer shadow-wealth-sm">
              {copied ? (<>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 border-y border-wealth-border py-4 mt-2">
                <div className="w-10 h-10 relative rounded-full overflow-hidden border border-wealth-border bg-wealth-surface-dim flex items-center justify-center flex-shrink-0">
                  {blog.author.avatar.startsWith("http") ? (<Image src={blog.author.avatar} alt={blog.author.name} fill className="object-cover"/>) : (<Sparkles className="w-5 h-5 text-wealth-accent"/>)}
                </div>
                <div>
            ...
            <div className="flex items-center gap-1 border-b border-wealth-border pb-px">
              <button onClick={() => setActiveSubTab("article")} className={`px-4 py-2.5 text-xs font-black uppercase tracking-wider border-b-2 cursor-pointer transition-all ${activeSubTab === "article"
            ...
            : "border-transparent text-wealth-secondary hover:text-wealth-primary"}`}>
              ...
              <button onClick={() => setActiveSubTab("report")} className={`px-4 py-2.5 text-xs font-black uppercase tracking-wider border-b-2 cursor-pointer transition-all ${activeSubTab === "report"
            ? "border-[#fe9800] text-wealth-primary"
            : "border-transparent text-wealth-secondary hover:text-wealth-primary"}`}>
            ...
            {activeSubTab === "article" ? (<div className="space-y-8 animate-fade-up">
                ...
                <div className="text-wealth-secondary leading-relaxed prose max-w-none prose-headings:font-display mt-8" dangerouslySetInnerHTML={{ __html: formatMarkdown(blog.content) }}/>
                ...
                <div className="p-6 bg-white border border-wealth-border rounded-3xl shadow-wealth-sm space-y-4">
                  ...
                  <div className="flex flex-col md:flex-row items-stretch justify-between gap-3 py-2">
                    {analytics.diagram.nodes.map((node, idx) => (<div key={idx} className="flex-1 flex flex-col md:flex-row items-center w-full">
                        ...
                        {idx < analytics.diagram.nodes.length - 1 && (<div className="my-1.5 md:my-0 md:mx-1 text-wealth-muted transform rotate-90 md:rotate-0 flex-shrink-0">
                ...
                return (<circle key={idx} cx="80" cy="80" r="50" fill="none" stroke={s.color} strokeWidth="20" strokeDasharray={circumference} strokeDashoffset={strokeOffset} className="transition-all duration-300 hover:opacity-80 cursor-pointer" style={{ strokeDasharray: `${(s.value / 100) * circumference} ${circumference}` }}/>);
            })}
                        </svg>
                      ...
                      <div className="space-y-2 flex-1 w-full">
                        {analytics.sectors.map((s, idx) => (<div key={idx} className="flex items-center justify-between text-[10px] font-bold">
                ...
                return (<g key={idx}>
                              <rect x={x} y={y} width="28" height={barHeight} fill={isPos ? "#10b981" : "#ef4444"} rx="4" className="transition-all duration-300 hover:opacity-85 cursor-pointer"/>
                              <text x={x + 14} y={isPos ? y - 6 : y + barHeight + 12} textAnchor="middle" fontSize="9" fontWeight="bold" className="font-sans fill-slate-700">
                                {isPos ? "+" : ""}{ret.value}%
                              </text>
                              <text x={x + 14} y="152" textAnchor="middle" fontSize="9" fontWeight="bold" className="font-sans fill-slate-400">
                                {ret.year}
                              </text>
                            </g>);
            })}
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-slate-100 text-xs">
                        {analytics.metrics.map((m, idx) => (<tr key={idx} className="hover:bg-slate-50/50 transition-colors">
          ...
          <div className="space-y-6">
            ...
            {blog.symbol && (<div className="bg-white border border-wealth-border rounded-3xl shadow-wealth-md p-6">
                <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-6">
                  ...
                  <div className="flex items-center gap-0.5 bg-wealth-surface-dim p-0.5 rounded-lg border border-slate-200">
                    {(["1H", "1D", "1W", "1M"] as const).map(time => (<button key={time} onClick={() => setChartTimeframe(time)} className={`px-2 py-1 rounded text-[9px] font-bold uppercase transition-all cursor-pointer ${chartTimeframe === time
                ...
                <div className="w-full h-44 relative">
                  <svg viewBox="0 0 600 200" className="w-full h-full overflow-visible" preserveAspectRatio="none">
                    ...
                    {area && (<path d={area} fill="url(#sidebarChartFill)"/>)}

                    
                    {line && (<path d={line} fill="none" stroke="#fe9800" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>)}

                    
                    {points.length > 0 && (<circle cx={points[points.length - 1].x} cy={points[points.length - 1].y} r="4.5" fill="#fe9800" stroke="#ffffff" strokeWidth="1.5" className="animate-pulse"/>)}
                  </svg>
                </div>
            ...
            {blog.symbol && (<div className="bg-white border border-wealth-border rounded-3xl shadow-wealth-md p-6 space-y-4">
