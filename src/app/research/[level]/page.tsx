...
function findLevelForSlug(slug: string): { level: CourseLevel; index: number } | null {
  const normalized = slug.toLowerCase().trim();

  // 1. Direct level.id match (e.g. "foundation", "product-knowledge")
  let index = courseLevels.findIndex((l) => l.id.toLowerCase() === normalized);
  if (index !== -1) return { level: courseLevels[index], index };

  // 2. Level number match (e.g. "level-1", "1")
  ...
  // 3. Module ID/Number match (e.g. "module-5", "5", "m5")
  ...
  if (moduleNumMatch) {
    ...
    if (targetModule) {
      index = courseLevels.findIndex(
        (l) => modNum >= l.moduleRange[0] && modNum <= l.moduleRange[1],
      );
      if (index !== -1) return { level: courseLevels[index], index };
    }
  }
