...
const slidesData = [
    {
        ...
        description: "The portfolio dashboard serves as the centerpiece of the app, offering users a real-time overview of their mutual fund investments. It consolidates key information, including total portfolio value, recent transactions, and performance trends, all in one place.",
    ...
    },
    {
        ...
        description: "One of the most significant barriers for mutual fund investors is the tedious onboarding process, particularly the CAN (Common Account Number) registration. The app addresses this with a streamlined, integrated approach.",
...
export function ScrollingFeatureShowcase() {
    return (<div className="w-full bg-[#FFFDF4] pt-8 pb-0 md:pt-12 md:pb-0 relative">
      ...
      <div className="relative z-10">
        <ScrollStack useWindowScroll={true} itemDistance={280} itemStackDistance={60} stackPosition="25%" scaleEndPosition="10%" baseScale={0.85} blurAmount={4}>
          {slidesData.map((slide, index) => (<ScrollStackItem key={index}>
              <div className="flex flex-col md:flex-row h-full w-full bg-white rounded-[40px] shadow-2xl shadow-black/5 overflow-hidden border border-gray-100">
                ...
                <div className="flex-1 p-6 sm:p-10 md:p-16 flex flex-col justify-center relative">
                  ...
                  {slide.subtitle && (<p className="text-lg text-gray-700 font-medium mb-4">
                      {slide.subtitle}
                    </p>)}
                  {slide.bullets && slide.bullets.length > 0 && (<ul className="space-y-3">
                      {slide.bullets.map((bullet, i) => (<li key={i} className="flex items-center text-lg text-gray-600 font-medium">
                ...
                <div className="hidden md:flex flex-1 relative min-h-full bg-gray-50 items-center justify-center p-8">
                  <div className="relative w-full h-[250px] sm:h-[300px] md:h-[500px]">
                    <Image src={slide.image} alt={slide.title} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-contain drop-shadow-xl"/>
                  </div>
                </div>
              </div>
            </ScrollStackItem>))}
