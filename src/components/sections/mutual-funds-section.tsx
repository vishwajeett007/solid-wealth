...
function getCategory(name: string): string {
    ...
    if (n.includes("small cap") || n.includes("smallcap"))
        return "Small Cap";
    if (n.includes("mid cap") || n.includes("midcap") || n.includes("emerging"))
        return "Mid Cap";
    if (n.includes("large cap") || n.includes("largecap") || n.includes("bluechip") || n.includes("index"))
        return "Large Cap";
    if (n.includes("hybrid") || n.includes("balanced"))
        return "Hybrid";
    if (n.includes("tech") || n.includes("sector") || n.includes("pharma") || n.includes("infra") || n.includes("digital"))
        return "Sectoral";
    return "Equity";
}
function getRisk(name: string, category: string): string {
    const n = name.toLowerCase();
    if (category === "Small Cap" || category === "Sectoral" || n.includes("opportunities") || n.includes("tactical"))
        return "High";
    if (category === "Hybrid" || n.includes("debt") || n.includes("conservative") || n.includes("liquid"))
        return "Low";
    return "Moderate";
}
      ...
      <div className="max-w-[1400px] mx-auto flex flex-col items-center">
        ...
        <div className="w-full relative flex flex-col-reverse md:flex-row items-center justify-center gap-6 md:gap-0 mb-16 max-w-[1400px] min-h-[48px]">
          ...
          <div className="flex flex-row overflow-x-auto no-scrollbar items-center justify-start md:justify-center gap-2 p-1.5 sm:p-2 bg-[#FFF9EA] rounded-[2rem] sm:rounded-full max-w-full z-20 w-full md:w-auto">
            {categories.map((cat) => (<button key={cat} onClick={() => { setSelectedCategory(cat); setActiveIndex(0); }} className={cn("px-4 sm:px-6 py-2 sm:py-2.5 rounded-full text-xs sm:text-sm font-bold transition-all cursor-pointer whitespace-nowrap", selectedCategory === cat
          ...
          <div className="relative md:absolute md:right-0 w-full md:max-w-xs h-12 flex-shrink-0 z-30">
            <div className="w-full h-full flex items-center bg-[#F8FAFC] border border-slate-200 focus-within:border-[#0B1F3A] focus-within:bg-white rounded-full absolute inset-0 px-4 transition-colors">
              ...
              <input type="text" placeholder="Search mutual funds..." value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setActiveIndex(0); }} className="w-full bg-transparent outline-none font-semibold text-[#0B1F3A] placeholder:text-gray-400 text-sm py-2"/>
              {searchQuery && (<button onClick={() => { setSearchQuery(""); setActiveIndex(0); }} className="text-gray-400 hover:text-gray-600 flex-shrink-0 cursor-pointer mr-1">
                  <X className="size-4"/>
                </button>)}
            </div>
          </div>
        ...
        {isLoading ? (<div className="relative w-full px-2 md:px-16 flex items-center mb-6">
            <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mx-auto">
              {Array.from({ length: 6 }).map((_, i) => (<div key={i} className="bg-white border border-[#EBEFF5] rounded-[32px] p-8 flex flex-col justify-between h-[380px] animate-pulse shadow-sm">
                  <div>
            ...
            <div className="relative w-full md:px-4 flex items-center mb-6">
              ...
              <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mx-auto">
                {pagesData[activeIndex]?.map((fund, index) => (<div key={`${activeIndex}-${index}`} className="bg-white border border-[#EBEFF5] rounded-[32px] p-8 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between h-full animate-in fade-in slide-in-from-bottom-4 duration-300">
                    <div>
                      ...
                      <div className="flex items-start justify-between gap-4 mb-1">
                        ...
                        <span className={cn("px-3 py-1 rounded-full text-xs font-bold flex-shrink-0", fund.risk === "High" && "bg-[#FFF0F0] text-[#FF4D4D]", fund.risk === "Moderate" && "bg-[#FFF8EB] text-[#FFA200]", fund.risk === "Low" && "bg-[#E6FCF5] text-[#00C853]")}>
                          {fund.risk}
                        </span>
                      </div>
                    ...
                    <div>
                      ...
                      <button onClick={() => setSelectedFund(fund)} className="w-full bg-[#0B1F3A] hover:bg-[#152e52] text-white py-3.5 rounded-full font-bold flex items-center justify-center gap-2 transition-all duration-200 group">
                        Invest Now
      ...
      {selectedFund && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setSelectedFund(null)}>
          <div className="bg-white rounded-3xl p-6 md:p-8 w-full max-w-lg shadow-2xl animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-6">
              ...
              <button onClick={() => setSelectedFund(null)} className="p-2 -mr-2 -mt-2 text-gray-400 hover:text-gray-900 bg-gray-50 hover:bg-gray-100 rounded-full transition-colors flex-shrink-0">
                <X className="size-5"/>
              </button>
            </div>
            ...
            <div className="flex gap-3">
              <button className="flex-1 bg-white border-2 border-gray-200 text-gray-700 font-bold py-3.5 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-colors" onClick={() => setSelectedFund(null)}>
                Close
              </button>
              <Link href="/invest" className="flex-[2] bg-[#fe9800] text-white font-bold py-3.5 rounded-xl hover:bg-orange-500 transition-colors flex items-center justify-center">
                Invest Now
