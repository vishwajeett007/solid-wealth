...
export function ReviewsSection() {
  ...
  // References for cards and their float wrappers
  const cardRefs = [
  ...
  const startFloating = () => {
    ...
    if (floatTweens.current.length > 0) return;
  ...
  const stopFloating = () => {
    ...
    // Smoothly return float wrappers to original Y positions
  ...
  useGSAP(
    () => {
      ...
      // Desktop Viewports: Premium Scroll-Driven Envelope Pull Animation
      mm.add("(min-width: 768px)", () => {
        const calculateOffsets = () => {
          // Clear current layout styles to measure fresh, accurate static positions
          gsap.set(cards, { clearProps: "all" });

          const envRect = envelopeRef.current?.getBoundingClientRect();
          if (!envRect) return [];

          // Find the center of the envelope pocket lip
          // Lip is roughly 61.8% down the height of the envelope image
          const envCenter = {
          ...
          return cards.map((card) => {
            ...
            return {
              ...
              // Offset slightly down so cards hide deep in the pocket initially
        ...
        const buildAnimation = () => {
          ...
          // Set initial stacked states deep inside the envelope
          ...
          // STAGE 1: Cards rise straight up out of the pocket lip (maintaining stacked X offsets)
          // Center card rises first
          ...
          // STAGE 2: Cards separate and settle smoothly into their final layout positions
          // Center card
          tl.to(
            cards[1],
            {
      ...
      // Mobile Viewports: Smooth sequential fade-in and slide-up (no pinning to prevent scroll jumps)
      ...
      <div className="relative w-full max-w-[1200px] flex-1 flex flex-col items-center justify-between px-4 md:px-8 mt-2 mb-0">
        ...
        >
          {reviewsData.map((review, idx) => {
            ...
            // On desktop, center card starts with higher stack index
            ...
            return (
              <div
                key={review.id}
                className="relative h-[360px] md:h-[340px] lg:h-[360px]"
              >
                <div
                  ref={cardRefs[idx]}
                  className={cn("absolute inset-0 w-full h-full", zIndexClass)}
        ...
        >
          ...
          <svg
            viewBox="0 0 1027 437"
            className="absolute inset-0 w-full h-full"
            preserveAspectRatio="none"
          >
            <polygon
              points="0,270 513.5,437 1027,270 1027,437 0,437"
              fill="white"
            />
          </svg>

          {/* Front pocket outline using bottom half of poster.png */}
          <div
            className="absolute inset-0 w-full h-full"
            style={{
              backgroundImage: `url(/poster.png)`,
          ...
          {/* Premium Gold/Orange Brand Badge */}
  ...
  return (
    <div
      className={cn(
        ...
        "hover:shadow-[0_24px_50px_rgba(15,26,44,0.08)] hover:scale-[1.03] hover:-translate-y-1.5",
        "transition-all duration-300 ease-out cursor-pointer flex flex-col justify-between h-full w-full"
      )}
    >
