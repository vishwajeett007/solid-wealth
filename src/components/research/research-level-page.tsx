...
function getTopicGroups(courseModule: CourseModule) {
  if (courseModule.subsections?.length) return courseModule.subsections;
      ...
      <section className="relative isolate overflow-hidden border-b border-wealth-dark-border bg-wealth-primary px-4 pb-16 pt-24 text-white sm:px-6 sm:pb-20 sm:pt-28 lg:px-8">
        ...
        <div className="mx-auto max-w-7xl">
          <nav
            ...
            className="flex flex-wrap items-center gap-2 text-xs font-semibold text-white/55"
          >
          ...
          <div className="mt-9 grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
            ...
            <Link
              className="inline-flex h-11 w-fit items-center gap-2 rounded-full border border-white/15 bg-white/[0.06] px-5 text-sm font-bold text-white transition hover:border-wealth-accent hover:text-wealth-accent"
              href="/research"
            >
      ...
      <section className="px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[260px_minmax(0,1fr)] xl:gap-12">
                ...
                <ol className="space-y-1.5">
                  {modules.map((courseModule) => (
                    <li key={courseModule.id}>
                      <a
                        className="group flex items-start gap-2.5 rounded-xl px-2.5 py-2 text-xs font-semibold leading-5 text-wealth-secondary transition hover:bg-wealth-accent-light/60 hover:text-wealth-primary"
          ...
          <div className="min-w-0">
              ...
              {modules.map((courseModule) => (
                ...
                >
                  <summary className="flex cursor-pointer list-none items-start gap-3 p-5 marker:hidden sm:items-center sm:gap-5 sm:p-7 [&::-webkit-details-marker]:hidden">
                    ...
                    <ChevronDown
                      aria-hidden="true"
                      className="mt-3 size-5 shrink-0 text-wealth-muted transition-transform duration-200 group-open:rotate-180 group-open:text-wealth-accent sm:mt-0"
                    />
                  </summary>
                  ...
                  <div className="border-t border-wealth-border/70 bg-[#fffdf8] px-4 py-6 sm:px-7 sm:py-8">
                    ...
                    <div className="space-y-8">
                      {getTopicGroups(courseModule).map((group, groupIndex) => (
                        <section key={group.title ?? `group-${groupIndex}`}>
                          ...
                          <div className="grid gap-3 xl:grid-cols-2">
                            {group.topics.map((topic, topicIndex) => (
                              <article
                                className="rounded-2xl border border-wealth-border/70 bg-white p-4 transition hover:border-wealth-accent/35 hover:shadow-wealth-sm sm:p-5"
                                id={`${courseModule.id}-topic-${groupIndex + 1}-${topicIndex + 1}`}
                                key={topic}
                              >
            ...
            <section className="mt-10 rounded-3xl border border-wealth-border/80 bg-white p-5 shadow-wealth-sm sm:p-7">
              ...
              <div className="mt-6 grid gap-3 md:grid-cols-3">
                {courseReferenceSources.map((source) => (
                  <a
                    className="group rounded-xl border border-wealth-border/70 bg-[#fffdf8] p-4 transition hover:border-wealth-accent/40"
                    href={source.href}
                    key={source.title}
                    rel="noreferrer"
                    target="_blank"
                  >
                    <span className="flex items-start justify-between gap-2">
                      <FileText
                        aria-hidden="true"
                        className="size-4 text-wealth-accent"
                      />
                      <ExternalLink
                        aria-hidden="true"
                        className="size-3.5 text-wealth-muted transition group-hover:text-wealth-accent"
                      />
                    </span>
            ...
            <nav
              aria-label="Course level pagination"
              className="mt-8 grid gap-3 sm:grid-cols-2"
            >
              {previousLevel ? (
                <Link
                  className="group rounded-2xl border border-wealth-border bg-white p-5 transition hover:border-wealth-accent/40 hover:shadow-wealth-sm"
              ...
              {nextLevel ? (
                <Link
                  className="group rounded-2xl border border-wealth-border bg-white p-5 text-right transition hover:border-wealth-accent/40 hover:shadow-wealth-sm"
