...
export function ResearchCourse() {
  ...
  const visibleLevels = courseLevels
    .filter(
      (level) => activeLevel === "all" || level.levelNumber === activeLevel,
    )
    .map((level) => ({
      ...level,
  ...
  const visibleModuleCount = visibleLevels.reduce(
  ...
  return (
    <div className="overflow-hidden bg-[#fffdf8]">
        ...
        <div className="mx-auto max-w-7xl">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
              <a
                className="group inline-flex h-12 items-center justify-center gap-2 rounded-full bg-wealth-primary px-6 text-sm font-bold text-white shadow-wealth-md transition hover:-translate-y-0.5 hover:bg-black"
                href="#curriculum"
              >
              ...
              <a
                className="inline-flex h-12 items-center justify-center gap-2 rounded-full border border-wealth-border bg-white px-6 text-sm font-bold text-wealth-primary transition hover:border-wealth-accent hover:text-wealth-accent"
                href="#course-format"
              >
            ...
            {[
              ...
              <div
                className={cn(
                  ...
                  index % 2 !== 0 && "border-l border-wealth-border/70",
                  index >= 2 && "border-t border-wealth-border/70 sm:border-t-0",
                  index === 2 && "sm:border-l",
                )}
                key={label}
              >
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {courseLevels.map((level, index) => {
              ...
              return (
                <Link
                  className={cn(
                    "group rounded-2xl border bg-white p-5 transition hover:-translate-y-1 hover:shadow-wealth-md",
                    theme.border,
                  )}
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <ArrowRight
                      aria-hidden="true"
                      className="size-4 text-wealth-muted transition-transform group-hover:translate-x-1 group-hover:text-wealth-accent"
                    />
                  </div>
      ...
      <section
        className="border-y border-wealth-border/70 bg-white px-4 py-16 sm:px-6 sm:py-20 lg:px-8"
        id="curriculum"
      >
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <label className="relative block w-full lg:max-w-md">
              ...
              <Search
                aria-hidden="true"
                className="absolute left-4 top-1/2 size-4 -translate-y-1/2 text-wealth-muted"
              />
              <input
                className="h-12 w-full rounded-xl border border-wealth-border bg-[#fffdf8] pl-11 pr-4 text-sm text-wealth-primary outline-none transition placeholder:text-wealth-muted focus:border-wealth-accent focus:ring-4 focus:ring-wealth-accent/10"
          ...
          <div className="mt-7 flex gap-2 overflow-x-auto pb-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <button
              className={cn(
                ...
                activeLevel === "all"
                  ...
                  : "border-wealth-border bg-white text-wealth-secondary hover:border-wealth-accent hover:text-wealth-accent",
              )}
            ...
            {courseLevels.map((level) => (
              <button
                ...
                key={level.id}
                onClick={() => setActiveLevel(level.levelNumber)}
                type="button"
              >
                {level.shortTitle}
              </button>
          ...
          <div className="mt-5 flex items-center justify-between gap-4 text-xs text-wealth-muted">
            ...
            {visibleModuleCount > 0 && (
              <button
                className="font-bold text-wealth-secondary transition hover:text-wealth-accent"
                onClick={() =>
                  setOpenModules(
          ...
          <div className="mt-8 space-y-12">
                ...
                >
                  <div className="mb-4 flex flex-col gap-3 border-l-4 border-wealth-accent pl-4 sm:flex-row sm:items-end sm:justify-between">
                    ...
                    <div className="sm:text-right">
                      ...
                      <Link
                        className="mt-1.5 inline-flex items-center gap-1 text-xs font-bold text-wealth-accent transition hover:gap-2"
                        href={`/research/${level.id}`}
                      >
                  ...
                  <div className="space-y-3">
                    {level.modules.map((courseModule) => {
                      ...
                      return (
                        <article
                          className={cn(
                            "scroll-mt-32 overflow-hidden rounded-2xl border bg-[#fffdf8] transition",
                            isOpen
                              ? `${theme.border} shadow-wealth-sm`
                              : "border-wealth-border/80 hover:border-wealth-accent/40",
                          )}
                          id={courseModule.id}
                          key={courseModule.id}
                        >
                          <button
                            aria-controls={`${courseModule.id}-content`}
                            aria-expanded={isOpen}
                            className="flex w-full items-start gap-3 p-4 text-left sm:items-center sm:gap-5 sm:p-5"
                            onClick={() =>
                              toggleModule(courseModule.moduleNumber)
                            }
                            type="button"
                          >
                            <span
                              className={cn(
                                "flex size-10 shrink-0 items-center justify-center rounded-xl text-xs font-extrabold tabular-nums sm:size-12 sm:text-sm",
                                theme.number,
                              )}
                            >
                              {courseModule.moduleNumber}
                            ...
                            <ChevronDown
                              aria-hidden="true"
                              className={cn(
                                "mt-2 size-5 shrink-0 text-wealth-muted transition-transform duration-200 sm:mt-0",
                                isOpen && "rotate-180 text-wealth-accent",
                              )}
                            />
                          </button>
                          ...
                          {isOpen && (
                            <div
                              className="border-t border-wealth-border/70 bg-white px-4 py-5 sm:px-6 sm:py-6"
                              id={`${courseModule.id}-content`}
                            >
                              <p className="mb-5 text-sm leading-6 text-wealth-secondary sm:hidden">
                                {courseModule.description}
                              </p>

                              {courseModule.subsections ? (
                                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                                  {courseModule.subsections.map((subsection) => (
                                    <div
                                      className="rounded-xl border border-wealth-border/70 bg-[#fffdf8] p-4"
                                      key={subsection.title}
                                    >
                                      {subsection.title && (
                                        <h4 className="mb-3 text-sm font-bold text-wealth-primary">
                                          {subsection.title}
                                        </h4>
                                      )}
                                      <ul className="space-y-2">
                                        {subsection.topics.map((topic) => (
                                          <li key={topic}>
                                            <Link
                                              className="group flex items-start gap-2 text-sm leading-5 text-wealth-secondary transition hover:text-wealth-accent"
                                              href={`/research/${level.id}#${courseModule.id}`}
                                            >
                                              <Check
                                                aria-hidden="true"
                                                className="mt-0.5 size-3.5 shrink-0 text-wealth-accent transition-transform group-hover:scale-110"
                                              />
                                              ...
                                              <ArrowRight
                                                aria-hidden="true"
                                                className="mt-0.5 size-3 shrink-0 opacity-0 transition-all group-hover:opacity-100 group-hover:translate-x-0.5 text-wealth-accent"
                                              />
                                            </Link>
                                          </li>
                                        ))}
            ...
            {visibleLevels.length === 0 && (
              <div className="rounded-2xl border border-dashed border-wealth-border bg-[#fffdf8] px-6 py-16 text-center">
                <Search
                  aria-hidden="true"
                  className="mx-auto size-7 text-wealth-muted"
                />
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-9 grid gap-4 md:grid-cols-2">
            {professionalDeepDives.map((deepDive, index) => {
              ...
              return (
                <article
                  className="rounded-2xl border border-wealth-border/80 bg-white p-5 shadow-wealth-sm sm:p-6"
                  key={deepDive.title}
                >
                  ...
                  <ul className="mt-5 grid gap-2 sm:grid-cols-2">
                    {deepDive.topics.map((topic) => (
                      <li key={topic}>
                        <Link
                          className="group flex items-start gap-2 text-xs leading-5 text-wealth-secondary transition hover:text-wealth-accent"
                          href="#curriculum"
                        >
                          <Check
                            aria-hidden="true"
      ...
      <section
        className="border-y border-wealth-border/70 bg-wealth-primary px-4 py-16 text-white sm:px-6 sm:py-20 lg:px-8"
        id="course-format"
      >
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
            ...
            <ol className="grid grid-cols-2 gap-3 sm:grid-cols-5">
              {courseDeliveryFormat.map((step, index) => (
                <li
